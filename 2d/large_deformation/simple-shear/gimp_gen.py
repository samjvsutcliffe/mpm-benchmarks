...
sim = utl.Simulation(title="simple_shear")
...
pmesh = utl.Mesh(dimensions=particle_dims,origin=(10,0), ncells=[x//r for x,r in zip(particle_dims,resolutions)],cell_type =node_type)
...
minp = np.min(sim.particles.positions,axis=0)
maxp = np.max(sim.particles.positions,axis=0)
shear_rate = 0.1
#for p in sim.particles.positions:
#    if any(p==minp) or any(p==maxp):
#        sim.add_velocity_condition(0,shear_rate * p[1],sim.entity_sets.create_set(lambda x,y: x==p[0] and y==p[1], typ="particle"))

for n in sim.mesh.nodes:
    sim.add_velocity_condition(1,0,sim.entity_sets.create_set(lambda x,y: x==n[0] and y==n[1], typ="node"))
    sim.add_velocity_condition(0,shear_rate * n[1],sim.entity_sets.create_set(lambda x,y: x==n[0] and y==n[1], typ="node"))
#sim.add_velocity_condition(0,shear_rate * p[1],sim.entity_sets.create_set(lambda x,y: x==p[0] and y==p[1], typ="particle"))
...
sim.analysis_params["velocity_update"] = False
